...
with tab1:
    ...
    with col2:
        ...
        if 'sequence' in st.session_state and st.session_state.sequence:
            ...
            if st.button("🔄 Duplicate Selected", use_container_width=True):
                if 'selected_email_idx' in st.session_state and st.session_state.selected_email_idx is not None:
            ...
            if st.button("🗑️ Delete Selected", use_container_width=True):
                if 'selected_email_idx' in st.session_state and st.session_state.selected_email_idx is not None and len(st.session_state.sequence) > 1:
                    idx = st.session_state.selected_email_idx
                    del st.session_state.sequence[idx]
                    st.success("✅ Deleted email!")
                    st.rerun()
                else:
                    st.warning("Cannot delete the last email or no email selected!")
        else:
    ...
    if 'sequence' in st.session_state and st.session_state.sequence:
        ...
        search_term = st.text_input("🔍 Search emails", placeholder="Search by subject...", key="email_search")
        ...
        if filtered_sequence:
                ...
                with st.form(f"edit_email_{actual_idx}", clear_on_submit=False):
                    ...
                    with col1:
                        if st.form_submit_button("💾 Save Changes", type="primary", use_container_width=True):
                            email['email_body'] = create_email_html_template(
                                new_content,
                                st.session_state.get('header_img', DEFAULT_HEADER_IMAGE), # Use current header_img from sidebar
                                st.session_state.get('footer_img', DEFAULT_FOOTER_IMAGE)  # Use current footer_img from sidebar
                            )
                    ...
                    with col3:
                        if st.form_submit_button("👁️ Preview", use_container_width=True):
                            # Temporarily store preview content to show in preview tab
                            st.session_state.preview_content = new_content 
                            st.session_state.preview_actual_idx = actual_idx
                            st.rerun()
            ...
            with edit_tab3:
                # Use stored preview content if available from the "Preview" button click
                preview_html_content = st.session_state.get('preview_content', "") if st.session_state.get('preview_actual_idx') == actual_idx else ""
                if not preview_html_content:
                    # Fallback to current content from email object if no preview button was clicked
                    content_match = re.search(r'<td style="padding: 40px 30px;">(.*?)</td>', email.get('email_body', ''), re.DOTALL)
...
<div style='text-align: center; color: #666; padding: 20px;'>
    ...
    <p style='font-size: 14px; margin-top: 10px;'>
        Features: Template Library • Analytics • Schedule Calculator • Bulk Operations • Validation • Google Sheets Integration
    </p>
</div>
""", unsafe_allow_html=True)
